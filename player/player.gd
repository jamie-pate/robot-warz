...
#		d = 0
	...
	var actual_max_speed = max_speed * 3.0 if Cheats.is_active('run') else max_speed
	...
	if (is_on_floor()):
		...
		if (dir.length() > 0.1):
			...
			if (hv.length() > actual_max_speed):
				hv = hv.normalized()*actual_max_speed
		else:
			if (air_idle_deaccel):
	...
	if (is_on_floor()):
		get_node("AnimationTreePlayer").blend2_node_set_amount("walk", hspeed/actual_max_speed)
